...
function pageRedirect(formObject) {
    ...
    if (formObject.response.id !== undefined) {
        redirectTo = '/purchase/order/details/' + formObject.response.id;
    } else {
...
function addRowToInvoiceItemsTable(recordObject, loadedFromUpdateOperation = false) {
    ...
    var hiddenItemId = '<input id="item_id" type="hidden" name="item_id[' + currentRowId + ']" class="form-control" value="' + recordObject.id + '">';
    ...
    var quantity = '<input id="qty" type="number" name="quantity[' + currentRowId + ']" class="form-control text-center" value="' + recordObject.quantity + '">';
    var itemLocation = '<input id="item_location" type="text" name="item_location[' + currentRowId + ']" class="form-control text-center text-end2"  value="' + recordObject.item_location + '">';

    var inputUnitPrice = '<input type="text" readonly name="purchase_price[' + currentRowId + ']" class="form-control text-center" value="' + _parseFix(recordObject.purchase_price) + '">';
    ...
    var inputSaveButton = '<button type="button" class="btn btn-outline-info row-save"><i class="bx bx-save me-0"></i></button>';
    ...
    newRow.append('<td>' + itemLocation + '</td>');
    newRow.append('<td id="status">' + recordObject.status + '</td>');
    newRow.append('<td>' + inputSaveButton + '</td>');
    newRow.append('<td class="d-none">' + inputTotal + '</td>');
...
$(document).on('click', '.row-save', function () {
    ...
    const t_id = row.find("#t_id").text();
    const item_id = row.find("#item_id").val();
    const qty = row.find("#qty").val();
    const item_location = row.find("#item_location").val();
    var url = baseURL + '/purchase/order/row-save';
    console.log(item_id);
    console.log(t_id);
    console.log(qty);
    console.log(item_location);
    console.log("save click: " + url);
    const data = {'t_id': t_id, 'qty': qty, 'item_location': item_location, 'item_id': item_id};
...
itemSearchInputBoxId.autocomplete({
    ...
    focus: function (event, ui) {
        if (ui.item !== undefined) {
            ...
            if (ui.item.purchase_price !== undefined) {
                searchedItemPrice = _parseFix(ui.item.purchase_price);
            }
        }
        return false;
    },
...
function updateOperation() {
    ...
    jsonObject.forEach((data, index) => {
        var dataObject = {
            ...
            item_location: data.item.item_location,
            status: data.status,
            taxList: taxList,
...
async function deletePaymentRequest(paymentId) {
    ...
    if (confirmed) {
        var url = baseURL + '/payment/purchase-order/delete/';
