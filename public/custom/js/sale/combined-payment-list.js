$(function() {
    ...
    */
    window.loadDatatables = function() {
        ...
        var table = tableId.DataTable({
            ...
            method:'get',
            ajax: {
                    url: baseURL+'/transaction/combined-payment/datatable-list',
                    data:{
        ...
        table.on('click', '.deleteRequest', function () {
              ...
              let transactionType = $(this).attr('data-transaction-type');

              deleteRequest(deleteId, transactionType);
   ...
   async function validateCheckedCheckbox(){
     ...
     * Call Delete Request
    */
    async function deleteRequest(id, transactionType) {
        const confirmed = await confirmAction();//Defined in ./common/common.js
        if (confirmed) {
            deleteRecord(id, transactionType);
        }
    }
   ...
   async function requestDeleteRecords(){
     ...
     * Single Data Delete
    */
    function deleteRecord(id, transactionType){
        const form = datatableForm;
        const formArray = {
            formId: form.attr("id"),
            csrf: form.find('input[name="_token"]').val(),
            _method: form.find('input[name="_method"]').val(),
            // Use the correct URL based on transaction type
            url: transactionType === 'sale_order' ? baseURL + '/payment/sale-order/delete/'+id : baseURL + '/payment/sale-invoice/delete/'+id,
            formObject : form,
