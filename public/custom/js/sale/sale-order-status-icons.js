...
class SaleOrderStatusIcons {
    ...
    initializeStatusSelects() {
        $('.sale-order-status-select, .order-status-select').each((index, element) => {
...
window.saleOrderStatusIcons = new SaleOrderStatusIcons();

// Auto-initialize when document is ready
$(document).ready(function() {
    // Initialize all status selects
    window.saleOrderStatusIcons.initializeStatusSelects();

    // Re-initialize on dynamic content load
    $(document).on('DOMNodeInserted', function(e) {
        if ($(e.target).hasClass('sale-order-status-select') || $(e.target).find('.sale-order-status-select').length > 0) {
            setTimeout(() => {
                window.saleOrderStatusIcons.initializeStatusSelects();
            }, 100);
        }
