...
function pageRedirect(formObject) {
    ...
    if (formObject.response.id !== undefined) {
        redirectTo = '/sale/order/details/' + formObject.response.id;
    } else {
        redirectTo = '/sale/order/list';
    }
    setTimeout(function () {
...
function addRowToInvoiceItemsTable(recordObject, loadedFromUpdateOperation = false) {
    ...
    var itemLocation = '<input id="item_location" readonly type="text" name="item_location[' + currentRowId + ']" class="form-control text-center"  value="' + recordObject.item_location + '">';
    var inputUnitPrice = '<input type="text" readonly name="sale_price[' + currentRowId + ']" class="form-control text-center" value="' + _parseFix(recordObject.sale_price) + '">';
...
$(document).on('click', '.row-save', function () {
    ...
    var url = baseURL + '/sale/order/row-save';
    // console.log(item_id);
    console.log(t_id);
    console.log(qty);
    // console.log(item_location);
    console.log("save click: " + url);
    const data = {'t_id': t_id, 'qty': qty};
 ...
 * */
$(document).on('change input', 'input[name="shipping_charge"]', function(){
    setBottomInvoiceTotal();
});
...
async function deletePaymentRequest(paymentId) {
    ...
    if (confirmed) {
        var url = baseURL + '/payment/sale-order/delete/';
