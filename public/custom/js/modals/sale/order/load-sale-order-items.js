$(function() {
    ...
    let openModal = $('#loadSaleOrderItemsModal');
    ...
    $(document).on('click', '#show_load_items_modal', function() {
        ...
        if(!partyId){
            iziToast.error({title: 'Error', layout: 2, message:"الرجاء اختيار العميل"});
            party.select2('open');
            return;
        }
    ...
    $(document).on('click', '.load-sale-order-items', function() {
        var partyId = party.val();
        var itemId = modalItemId.val() || '';
        var url = baseURL + `/sale/order/order-items/`+partyId+`/`;
        ajaxGetRequest(url ,itemId, 'sale-order-items');
    });
    ...
    $(document).ready(function () {
         ...
         */
        initSelect2ItemList($('#loadSaleOrderItemsModal'));
    });
