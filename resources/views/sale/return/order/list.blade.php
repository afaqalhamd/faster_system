                ...
                <div class="card-header px-4 py-3 d-flex justify-content-between">
                    ...
                    <div class="d-flex gap-2">
                        @can('purchase.order.import')
                            <a href="{{ route('import.sale-return') }}" class="btn btn-outline-primary px-5"><i
                                    class="bx bx-import me-1"></i> {{ __("app.oi") }}</a>
                        @endcan
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-3">
                            ...
                            <select class="party-ajax form-select" data-party-type='supplier'
                                    data-placeholder="Select Customer" id="party_id" name="party_id"></select>
                        </div>
                        <div class="col-md-3">
                            <x-label for="user_id" name="{{ __('user.user') }}"/>
                            <x-dropdown-user selected="" :showOnlyUsername='true'
                                             :canViewAllUsers="auth()->user()->can('purchase.order.can.view.other.users.purchase.orders')"/>
                        </div>
                        <div class="col-md-3">
                            <x-label for="from_date" name="{{ __('app.from_date') }}"/>
                            <a tabindex="0" class="text-primary" data-bs-toggle="popover" data-bs-trigger="hover focus"
                               data-bs-content="Filter by Order Date"><i class="fadeIn animated bx bx-info-circle"></i></a>
                    ...
                    <form class="row g-3 needs-validation" id="datatableForm"
                          action="{{ route('purchase.order.delete') }}" enctype="multipart/form-data">
                        {{-- CSRF Protection --}}
                        @csrf
                        @method('POST')
