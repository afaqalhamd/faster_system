...
class ImportController extends Controller
{
    ...
    /**
     * Import New Items from Excel Sheet
     * @return JsonResponse
     * */
    public function importItems(Request $request)
    {
        ...
        try {
            ...
            foreach ($data as $row) {
                ...
                $volume = trim($row[5] ?? '0');
                $price = trim($row[6] ?? '0');
                $cargoFee = trim($row[7] ?? '0');
                $note = trim($row[8]);
                $custNum = trim($row[9]);
                $custNumNew = trim($row[10]);
                $imageUrl = trim($row[11]);
                $image2 = trim($row[12]);
                $image3 = trim($row[13]);
                $item_location = trim($row[14] ?? '');
                $openingStockQty = trim($row[15] ?? '0');

                $recordDetails = "Sheet:0, Row:" . ($i);
                ...
                $validator = Validator::make([
                    ...
                    'asin' => 'nullable|string|max:1300',
                    'price' => 'required|numeric|min:0',
                    'weight' => 'nullable|numeric|min:0',
                    'volume' => 'nullable|numeric|min:0',
                    'cargoFee' => 'nullable|numeric|min:0',
                    'imageUrl' => 'nullable|string|max:800',
                    'openingStockQty' => 'nullable|numeric|min:0',
                ]);

                if ($validator->fails()) {
                    throw new \Exception(__('app.validation_error') . ' ' . $recordDetails . '\n' . implode('\n', $validator->errors()->all()));
                }
                ...
                // Check if item with SKU exists, update if found, create if not
                ...
                if (!empty($openingStockQty) && $openingStockQty > 0) {
                    // Check and delete existing opening stock if any
                    if ($this->itemTransactionService->hasOpeningStock($item->id)) {
                        $this->itemTransactionService->deleteOpeningStock($item->id);
                    }
                    $transactionResponse = $this->itemTransactionService->recordItemTransactionEntry($item, [
                        'item_id' => $item->id,
                        'transaction_date' => Carbon::now()->format('Y-m-d'),
                        'warehouse_id' => $request->warehouse_id, // Main warehouse
                    ...
                    //Update Account
                    $this->accountTransactionService->itemOpeningStockTransaction($item);
                    $transactionCollection->push(['itemModel' => $item, 'itemName' => $item->name, 'transactionId' => $transactionResponse->id]);
                }
            }
     ...
     * @return JsonResponse
     * */
    public function importItems2(Request $request)
    {
        $file = $request->file('excel_file');
